...
const MapView: React.FC<MapViewProps> = ({
      ...
      try {
        ...
        markerElement.innerHTML = `
          <div class="relative cursor-pointer transform transition-all duration-200 hover:scale-110 ${isSelected ? 'z-50' : ''}">
            <div class="w-12 h-12 rounded-full border-2 border-white shadow-brand flex items-center justify-center transition-all duration-200 ${colorClass} ${isSelected ? 'scale-125 shadow-brand-xl' : ''}">
  ...
  ? '<path d="M8 22h8"/><path d="M7 10h10"/><path d="M12 15v7"/><path d="M18 4H6v6a6 6 0 1 0 12 0V4Z"/>'
            ...
            <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 bg-brand-dark-900 text-white text-sm rounded-brand whitespace-nowrap opacity-0 transition-opacity duration-200 pointer-events-none ${isSelected ? 'opacity-100' : ''}">
              ${place.name}
      ...
      {isCalculatingRoute && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-brand-dark-900 dark:text-brand-dark-100">Calculando ruta...</span>
          </div>
        </div>
        </div>
      )}
